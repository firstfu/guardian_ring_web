...
export default function PhoneCallImage({ showDemo = false, className = "" }: PhoneCallImageProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/* 展示標籤 */}
      {showDemo && (
        <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-blue-600 text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg">真實的來電體驗</div>
      )}
    </div>
  );
}
